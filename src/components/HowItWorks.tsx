...
export const HowItWorks: React.FC<HowItWorksProps> = ({ onOpenAudit }) => {
  ...
  return (
    <section ref={sectionRef} id="how-it-works" className="py-24 bg-[#F7E8A1] relative overflow-hidden border-b border-[#CCA25A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="hidden md:block absolute top-8 bottom-8 left-1/2 -translate-x-1/2 w-0.5 bg-[#CCA25A]/40 z-0">
            <div
              ref={lineRef}
              className="w-full h-full bg-[#CCA25A] origin-top scale-y-0"
            />
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full md:w-1/2">
                    <div className="bg-[#FFF5B8] border-2 border-[#CCA25A] p-6 sm:p-8 rounded-3xl shadow-lg hover:border-[#2C200B] transition-all group">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-xs font-mono font-bold text-[#FFF5B8] bg-[#CCA25A] px-2.5 py-1 rounded-lg border border-[#2C200B]">
                          PHASE {item.step}
                        </span>
                        <div className="w-10 h-10 rounded-xl bg-[#F7E8A1] border border-[#CCA25A] flex items-center justify-center text-[#2C200B] group-hover:scale-110 transition-transform">
                          <Icon className="w-5 h-5 text-[#CCA25A]" />
                        </div>
                      </div>

                      <h3 className="text-xl font-bold font-heading text-[#2C200B] mb-3">
                        {item.title}
                      </h3>

                      <p className="text-[#3A2A0E] text-sm leading-relaxed mb-6 font-medium">
                        {item.description}
                      </p>

                      <div className="space-y-2 pt-2 border-t border-[#CCA25A] font-medium">
                        {item.highlights.map((h, i) => (
                          <div key={i} className="flex items-center gap-2 text-xs text-[#2C200B]">
                            <CheckCircle2 className="w-3.5 h-3.5 text-[#CCA25A] shrink-0" />
                            <span>{h}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* Center Node Badge */}
                  <div className="w-12 h-12 rounded-2xl bg-[#CCA25A] border-2 border-[#2C200B] text-[#FFF5B8] font-bold font-mono text-sm flex items-center justify-center shrink-0 shadow-lg z-20">
                    {item.step}
                  </div>
        ...
        <div className="mt-16 text-center">
          <button
            onClick={onOpenAudit}
            className="px-8 py-4 bg-[#CCA25A] hover:bg-[#A8813A] border border-[#2C200B] text-[#FFF5B8] font-bold rounded-xl shadow-lg transition-all inline-flex items-center gap-2"
          >
            <span>Start Phase 01 Audit Now</span>
            <ArrowRight className="w-4 h-4 text-[#FFF5B8]" />
          </button>
        </div>
      </div>
    </section>
