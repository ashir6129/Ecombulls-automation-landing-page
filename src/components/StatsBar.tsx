...
export const StatsBar: React.FC = () => {
  ...
  return (
    <section className="py-16 bg-surface-elevated/40 border-y border-border relative overflow-hidden">
          ...
          {stats.map((stat, idx) => {
            ...
            return (
              <div
                key={idx}
                ref={stat.ref}
                className="p-6 rounded-2xl bg-surface border border-border hover:border-border-glow transition-all space-y-2 group text-center lg:text-left"
              >
                <div className="w-10 h-10 rounded-xl bg-surface-elevated border border-border flex items-center justify-center text-accent mx-auto lg:mx-0 group-hover:scale-110 transition-transform">
